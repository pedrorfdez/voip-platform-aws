...
envsubst '${DB_HOST} ${DB_PORT} ${DB_NAME} ${DB_PASSWORD}' \
...
DB_URL="postgresql://sipadmin:${DB_PASSWORD}@${DB_HOST}:${DB_PORT}/${DB_NAME}"
...
psql "$DB_URL" -f /etc/kamailio/db/schema.sql
psql "$DB_URL" -f /etc/kamailio/db/seed.sql

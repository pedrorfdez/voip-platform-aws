...
# The ECS API rejects a task definition with the same containerPort registered under two
# different protocols. TCP and UDP for port 5060 must be split into separate task
# definitions and separate services.
locals {
  sip_container_base = {
    name      = "sip"
...
resource "aws_ecs_task_definition" "sip_udp" {
  family                   = "${var.name_prefix}-sip-udp"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = var.task_cpu
  memory                   = var.task_memory
  ...
  tags = merge(var.tags, { Name = "${var.name_prefix}-sip-udp-task" })
}
...
resource "aws_ecs_service" "sip_udp" {
  name                              = "${var.name_prefix}-sip-udp"
  cluster                           = aws_ecs_cluster.this.id
  task_definition                   = aws_ecs_task_definition.sip_udp.arn
  desired_count                     = var.desired_count
  launch_type                       = "FARGATE"
  enable_execute_command            = var.enable_execute_command
  health_check_grace_period_seconds = var.health_check_grace_period_seconds
  ...
  load_balancer {
    target_group_arn = var.tg_sip_udp_arn
    container_name   = "sip"
    container_port   = 5060
  }
  ...
  tags = merge(var.tags, { Name = "${var.name_prefix}-sip-udp" })
...
resource "aws_appautoscaling_target" "sip_udp" {
  max_capacity       = var.max_capacity
  min_capacity       = var.min_capacity
  resource_id        = "service/${aws_ecs_cluster.this.name}/${aws_ecs_service.sip_udp.name}"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"
}

resource "aws_appautoscaling_policy" "sip_udp_cpu" {
  name               = "${var.name_prefix}-sip-udp-cpu"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.sip_udp.resource_id
  scalable_dimension = aws_appautoscaling_target.sip_udp.scalable_dimension
  service_namespace  = aws_appautoscaling_target.sip_udp.service_namespace
